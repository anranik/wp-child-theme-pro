...
class WP_Child_Theme_Customizer {
    ...
    public function customize_register($wp_customize) {
        ...
        $wp_customize->add_setting('wpchild_custom_js', array(
            'default' => '',
            'transport' => 'refresh',
            'sanitize_callback' => 'wp_filter_nohtml_kses',
        ));
        
        // Add custom JS control
        $wp_customize->add_control('wpchild_custom_js', array(
            'label' => __('Custom JavaScript', 'wp-child-theme-pro'),
            'section' => 'wpchild_customizer_section',
            'settings' => 'wpchild_custom_js',
            'type' => 'textarea',
        ));
    ...
    public function save_custom_css($theme, $css) {
        ...
        if (!file_exists($style_css)) {
            return new WP_Error('no_style_css', __('style.css not found in theme.', 'wp-child-theme-pro'));
        }
    ...
    public function save_custom_js($theme, $js) {
        ...
        // Add header comment
        $js_content = "/**\n";
        $js_content .= " * Custom JS for " . $theme . "\n";
    ...
    /**
     * Apply customizer settings to theme
     *
     * @param string $theme Theme slug
    ...
    public function apply_customizer_settings($theme) {
        ...
        // Save custom CSS to theme if not empty
        if (!empty($custom_css)) {
            return $this->save_custom_css($theme, $custom_css);
        }
        
        // Save custom JS to theme if not empty
        if (!empty($settings['wpchild_custom_js'])) {
            return $this->save_custom_js($theme, $settings['wpchild_custom_js']);
        }
